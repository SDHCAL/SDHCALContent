...
#include "SDHCALUtility/KDTreeLinkerAlgoT.h"

#include "SDHCALObjects/SDHCALTrack.h"
...
pandora::StatusCode TrackClusterAssociationAlgorithm::Run()
{
    ...
    for (const pandora::Track *const pTrack : trackVector)
    {
        ...
        SDHCALTrackStates trackStates;
        const SDHCALTrack* sdhcalTrack = dynamic_cast<const SDHCALTrack*>(pTrack);
        if( sdhcalTrack )
        {
            trackStates = sdhcalTrack->GetTrackStates();
        }
        else
        {
        ...
        for (auto const& trackState : trackStates)
        {
            ...
            for (const pandora::Cluster *const pCluster : nearbyClusterList)
            {
                ...
                if (pandora::STATUS_CODE_SUCCESS != sdhcal_content::ClusterHelper::GetTrackClusterDistance(&trackState, pCluster, m_maxSearchLayer, m_parallelDistanceCut,
