...
namespace sdhcal_content
{
  ...
  void VisualMonitoringAlgorithm::VisualizeConnectors(const pandora::CaloHitList *const pCaloHitList, std::string name, TEveElement* parent, ::Color color) const
  {
    if(this->GetPandora().GetSettings()->IsMonitoringEnabled())
    {
          ...
          endIter != iter ; ++iter)
      {
        ...
        const ConnectorList &connectorList(SDHCALContentApi::GetConnectorList(pCaloHit, FORWARD_DIRECTION));
  ...
  void VisualMonitoringAlgorithm::VisualizeClusterDirection(const pandora::Cluster* const pCluster, std::string /* name */, TEveElement* parent, ::Color color) const
  {
    if(this->GetPandora().GetSettings()->IsMonitoringEnabled())
    {
	  ...
	  const sdhcal_content::SDHCALCluster* const pSDHCALCluster = dynamic_cast<const sdhcal_content::SDHCALCluster *const>(pCluster);

	  pandora::CartesianVector clusterCOG = pSDHCALCluster->GetIntercept();
	  pandora::CartesianVector clusterAxis = pSDHCALCluster->GetAxis();

	  //std::cout << "=================================== pSDHCALCluster: " << pSDHCALCluster << ", " << clusterCOG.GetX()
