...
StatusCode CheatingClusterCleaningAlgorithm::Run()
{
    ...
    for (ClusterList::const_iterator itCluster = pClusterList->begin(), itClusterEnd = pClusterList->end(); itCluster != itClusterEnd; ++itCluster )
    {
        try
        {
                ...
                for (pandora::CaloHitList::const_iterator hitIter = itLyr->second->begin(), hitIterEnd = itLyr->second->end(); hitIter != hitIterEnd; ++hitIter)
                {
                    try
                    {
                        ...
                        if (pMainMCParticle != pMCParticle)
                        {
                            ...
                            PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, SDHCALContentApi::RemoveFromCluster(*this, pCluster, pCaloHit));
        
							const sdhcal_content::CaloHit *const pSDHCALCaloHit = dynamic_cast<const sdhcal_content::CaloHit *const>( pCaloHit );

                            if(NULL == pSDHCALCaloHit) continue;
							...
							//pSDHCALCaloHit->GetDensity();
							float fwdConnection = 0.;
							float bakConnection = 0.;

							const ConnectorList &fwdConnectorList(SDHCALContentApi::GetConnectorList(pSDHCALCaloHit, FORWARD_DIRECTION));
							const ConnectorList &bakConnectorList(SDHCALContentApi::GetConnectorList(pSDHCALCaloHit, BACKWARD_DIRECTION));
							...
							float hitEnergy = pSDHCALCaloHit->GetInputEnergy();
			...
			PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, SDHCALContentApi::AddToCluster(*this, pCluster, &savedHits));
