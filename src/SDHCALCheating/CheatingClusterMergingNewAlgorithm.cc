	...
	for(auto it = mcpClusterListMap.begin(); it != mcpClusterListMap.end(); ++it)
	{
	    ...
	    if(clusterVector.size()>1) 
		{
			...
			for(int i = 1; i < clusterVector.size(); ++i)
			{
				...
				auto pSDHCALFirstCluster = SDHCALContentApi::Modifiable(dynamic_cast<const sdhcal_content::SDHCALCluster*>(firstCluster));
				auto pSDHCALCluToMerge = SDHCALContentApi::Modifiable(dynamic_cast<const sdhcal_content::SDHCALCluster*>(cluToMerge));

				float clusterAngle1 = ClusterHelper::GetClusterAxisStartingPointAngle(pSDHCALFirstCluster);
				float clusterAngle2 = ClusterHelper::GetClusterAxisStartingPointAngle(pSDHCALCluToMerge);

				float cluster1Time = ClusterHelper::GetAverageTime(pSDHCALFirstCluster);
				float cluster2Time = ClusterHelper::GetAverageTime(pSDHCALCluToMerge);
				...
				SDHCALContentApi::MergeAndDeleteClusters(*this, firstCluster, cluToMerge);
			}
