...
namespace sdhcal_content
{
  ...
  void NeutralFragmentMergingAlgorithm::SearchProperClusters(const pandora::Track* pTrack, SDHCALCluster* startingCluster)
  {
#if __DEBUG__
  ...
  pandora::StatusCode NeutralFragmentMergingAlgorithm::CleanClusterForMerging(std::vector<SDHCALCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode NeutralFragmentMergingAlgorithm::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
