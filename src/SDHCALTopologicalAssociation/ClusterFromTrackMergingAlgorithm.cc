...
namespace sdhcal_content
{
  ...
  void ClusterFromTrackMergingAlgorithm::SearchProperClusters(const pandora::Track* pTrack, SDHCALCluster* startingCluster, 
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm::CleanClusterForMerging(std::vector<SDHCALCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
		...
		SDHCALCluster* bestCluster = nullptr;
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
