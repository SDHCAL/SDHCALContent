...
namespace sdhcal_content
{
  ...
  void ClusterFromTrackMergingAlgorithm2::SearchProperClusters(const pandora::Track* pTrack, SDHCALCluster* startingCluster, 
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm2::CleanClusterForMerging(std::vector<SDHCALCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm2::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
