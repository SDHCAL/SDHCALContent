...
namespace sdhcal_content
{
  pandora::StatusCode ClusterFromTrackMergingAlgorithm3::Run()
  {
	...
	for(auto track : *pTrackList)
	{
		...
		auto associatedCluster = SDHCALContentApi::Modifiable(dynamic_cast<const sdhcal_content::SDHCALCluster*>(clu));
	...
	if(m_mergePhotonClusters)
	{
		for(auto photonCandidate : photonCandidates)
	    {
	    	...
	    	auto sdhcalCluster = SDHCALContentApi::Modifiable(dynamic_cast<const sdhcal_content::SDHCALCluster*>(photonCandidate));
	    	std::vector<SDHCALCluster*> properClusters;
	    	SearchProperClusters(sdhcalCluster, properClusters);
	    }
	}
  ...
  void ClusterFromTrackMergingAlgorithm3::SearchProperClusters(SDHCALCluster* startingCluster, 
  ...
  void ClusterFromTrackMergingAlgorithm3::SearchProperClusters(const pandora::Track* pTrack, SDHCALCluster* startingCluster, 
		  std::vector<sdhcal_content::SDHCALCluster*>& properClusters)
  {
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm3::CleanClusterForMerging(std::vector<SDHCALCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm3::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
