...
namespace sdhcal_content
{
  ...
  void ChargedFragmentsMergingAlgorithm::SearchProperClusters(const pandora::Track* /* pTrack */, SDHCALCluster* startingCluster, 
...
//				  closestDistance < m_maxClosestClusterDistance ||  // very close cluster
  ...
  bool ChargedFragmentsMergingAlgorithm::IsClusterAxesCompatible(const SDHCALCluster* startingCluster, const SDHCALCluster* nearbyCluster, float& axesDistance)
  {
  ...
  pandora::StatusCode ChargedFragmentsMergingAlgorithm::CleanClusterForMerging(std::vector<SDHCALCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode ChargedFragmentsMergingAlgorithm::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
