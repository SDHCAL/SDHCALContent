...
namespace sdhcal_content
{
  pandora::StatusCode PointingClusterAssociationNewAlgorithm::Run()
  {
	...
	for(int i = 0; i < trackStartingClusters.size(); ++i)
	{
		...
		std::vector<SDHCALCluster*> properClusters;
	...
	for(int i = 0; i < trackStartingClusters.size(); ++i)
	{
		...
		std::set<SDHCALCluster*> allClustersToMerge;
		...
		for(auto& cluToMerge : allClustersToMerge)
		{
			...
			if(cluToMerge != nullptr)
			{
				...
				SDHCALContentApi::MergeAndDeleteClusters(*this, trackStartCluster, cluToMerge);
			}
		}
	}
...
//#define __DEBUG__ 1
  ...
  void PointingClusterAssociationNewAlgorithm::SearchProperClusters(SDHCALCluster* startingCluster, 
		  std::vector<sdhcal_content::SDHCALCluster*>& properClusters)
  {
	  ...
	  for(int iClu = 0; iClu < properClusters.size(); ++iClu)
	  {
		  ...
		  std::vector<SDHCALCluster*> clusters;
  ...
  sdhcal_content::SDHCALCluster* PointingClusterAssociationNewAlgorithm::GetMainCluster(const pandora::CaloHitVector& caloHitVector)
  {
  ...
  pandora::StatusCode PointingClusterAssociationNewAlgorithm::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
