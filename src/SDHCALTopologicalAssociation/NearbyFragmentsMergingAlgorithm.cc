...
namespace sdhcal_content
{
  pandora::StatusCode NearbyFragmentsMergingAlgorithm::Run()
  {
	...
	for(auto cluster : m_clustersToMerge)
	{
		...
		auto sdhcalCluster = SDHCALContentApi::Modifiable(dynamic_cast<const sdhcal_content::SDHCALCluster*>(cluster));
		std::vector<SDHCALCluster*> properClusters;
		SearchProperClusters(sdhcalCluster, properClusters);

		if(properClusters.size()>0)
		{
			sdhcalCluster->SetRoot();
		}
	}
  ...
  void NearbyFragmentsMergingAlgorithm::MergeFragmentsByMC(std::vector<SDHCALCluster*>& fragmentsToMerge) 
  {
  ...
  void NearbyFragmentsMergingAlgorithm::SearchProperClusters(SDHCALCluster* startingCluster, 
  ...
  pandora::StatusCode NearbyFragmentsMergingAlgorithm::CleanClusterForMerging(std::vector<SDHCALCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode NearbyFragmentsMergingAlgorithm::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
        ...
        clusterEndIter != clusterIter ; ++clusterIter)
    {
	  ...
	  if(m_resetCluster) sdhcalCluster->Reset();
