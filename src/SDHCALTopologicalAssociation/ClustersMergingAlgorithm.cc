...
namespace sdhcal_content
{
  pandora::StatusCode ClustersMergingAlgorithm::Run()
  {
	...
	std::map<SDHCALCluster*, std::set<SDHCALCluster*>> mergingMap;
	...
	for(auto& mergingMapIter :  mergingMap)
	{
		...
		for(auto& clusterToMerge : clustersToMerge)
		{
			...
			std::cout << " @@@@@@ merge: " << clusterToEnlarge << ", E: " << clusterToEnlarge->GetHadronicEnergy() 
				...
				<< std::endl;
#endif
			SDHCALContentApi::MergeAndDeleteClusters(*this, clusterToEnlarge, clusterToMerge);
		}
	}
  ...
  pandora::StatusCode ClustersMergingAlgorithm::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
