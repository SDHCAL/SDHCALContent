...
namespace sdhcal_content
{
  pandora::StatusCode ChargedFragmentsMergingAlgorithm3::Run()
  {
	...
	std::vector<SDHCALCluster*> clustersToMerge;
	...
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		...
		std::vector<sdhcal_content::SDHCALCluster*> nearbyClusters;
  ...
  void ChargedFragmentsMergingAlgorithm3::MakeMergingMap(std::vector<SDHCALCluster*>& clustersToMerge,
		  std::vector<SDHCALCluster*>& clusterVector, std::map<const pandora::Cluster*, pandora::ClusterList>& clustersMergingMap)
  {
	...
	for(auto& pCluster : clustersToMerge)
	{
		...
		std::vector<sdhcal_content::SDHCALCluster*> veryCloseClusters;
  ...
  void ChargedFragmentsMergingAlgorithm3::MergeClusters(std::map<const pandora::Cluster*, pandora::ClusterList>& clustersMergingMap)
  {
	  for(auto& iter : clustersMergingMap)
	  {
		  ...
		  for(auto& cluster : clustersToMerge)
		  {
			 ...
			 SDHCALContentApi::MergeAndDeleteClusters(*this, mainCluster, cluster);
		  }
	  }
  }
  ...
  {
	  auto chClu = dynamic_cast<const sdhcal_content::SDHCALCluster*>(chargedCluster);
	  auto frg = dynamic_cast<const sdhcal_content::SDHCALCluster*>(fragment);
  ...
  bool ChargedFragmentsMergingAlgorithm3::CheckNearbyClusterWithCharge(const sdhcal_content::SDHCALCluster* pCluster, std::vector<sdhcal_content::SDHCALCluster*>& nearbyClusters, int charge)
  {
			  ...
			  if(nearbyCluster->GetAssociatedTrackList().size() > 0)
				...
				if(innerLayer==1 && outerLayer < 40) 
				{
					...
					if(passCheck)
					{
					    ...
					    {
					    	const sdhcal_content::CaloHit *const pSDHCALCaloHit(dynamic_cast<const sdhcal_content::CaloHit *const>(hit));
							const sdhcal_content::ConnectorList& backwardConnectorList = 
								SDHCALContentApi::GetConnectorList(pSDHCALCaloHit, sdhcal_content::BACKWARD_DIRECTION);
							const sdhcal_content::ConnectorList& forwardConnectorList = 
								SDHCALContentApi::GetConnectorList(pSDHCALCaloHit, sdhcal_content::FORWARD_DIRECTION);

							std::cout << "  hit on 1st layer: " << pSDHCALCaloHit << ", bakConnector: " << backwardConnectorList.size() 
  ...
  void ChargedFragmentsMergingAlgorithm3::SearchProperClusters(SDHCALCluster* startingCluster, 
		  ...
		  std::vector<float>& distances)
  {
	  ...
	  std::multimap<float, SDHCALCluster*> clusterDistanceMap;
	  ...
	  for(int i = 0; i < nearbyClusters.size(); ++i)
	  {
		  ...
		  {
		      clusterDistanceMap.insert( std::pair<float, SDHCALCluster*>(closestDistance, nearbyCluster) );
		  }
		  else
		  {
	          if(m_debugOutput2) 
  ...
  void ChargedFragmentsMergingAlgorithm3::GetNearbyClusters(pandora::Cluster* cluster, 
		  const std::vector<sdhcal_content::SDHCALCluster*>& clusterVector, std::vector<sdhcal_content::SDHCALCluster*>& clustersInRange)
  {
      pandora::CartesianVector centroid(0., 0., 0.);
	  ...
	  for(int i = 0; i < clustersInRange.size(); ++i)
	  {
		  ...
		  clusterDistanceMap.insert( std::pair<float, SDHCALCluster*>(distances.at(i), clusterInRange) );
	  }
  ...
  pandora::StatusCode ChargedFragmentsMergingAlgorithm3::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
