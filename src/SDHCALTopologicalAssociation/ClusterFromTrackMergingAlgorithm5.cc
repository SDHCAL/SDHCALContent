...
namespace sdhcal_content
{
  ...
  void ClusterFromTrackMergingAlgorithm5::SearchProperClusters(const pandora::Track* pTrack, SDHCALCluster* startingCluster, 
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm5::CleanClusterForMerging(std::vector<SDHCALCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm5::GetEligibleClusters(std::vector<SDHCALCluster*>& clusterVector) const
  {
	clusterVector.clear();
