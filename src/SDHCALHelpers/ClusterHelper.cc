...
namespace sdhcal_content
{
  ...
  float ClusterHelper::GetClusterAxisStartingPointAngle(const pandora::Cluster *const pCluster)
  {
	  ...
	  const SDHCALCluster* pSDHCALCluster = SDHCALContentApi::Modifiable(dynamic_cast<const sdhcal_content::SDHCALCluster*>(pCluster));

	  if(pSDHCALCluster == nullptr) return angleAxisStartingPoint;

	  auto& clusterAxis = pSDHCALCluster->GetAxis();
	  auto& clusterStartingPoint = pSDHCALCluster->GetStartingPoint();
  ...
  pandora::OrderedCaloHitList ClusterHelper::GetOrderedConnectedCaloHitList(const pandora::Cluster *const pCluster)
  {
	  ...
	  for(auto& pCaloHit : caloHitList)
	  {
		  ...
		  if(!SDHCALContentApi::HasAnyConnection(pSDHCALCaloHit)) continue;
  ...
  unsigned int ClusterHelper::GetClusterConnectorNumber(const pandora::Cluster *const pCluster)
  {
	  ...
	  {
		  const sdhcal_content::CaloHit *const pCaloHit(dynamic_cast<const sdhcal_content::CaloHit *const>(caloHit));
		  const sdhcal_content::ConnectorList& forwardConnectorList = 
			  SDHCALContentApi::GetConnectorList(pCaloHit, sdhcal_content::FORWARD_DIRECTION);
  ...
  pandora::StatusCode ClusterHelper::GetMainClusterHits(const pandora::Cluster *const pCluster, pandora::CaloHitList& mainClusterHits)
  {
	  ...
	  {
		  const sdhcal_content::CaloHit *const pCaloHit(dynamic_cast<const sdhcal_content::CaloHit *const>(caloHit));
		  if(!SDHCALContentApi::HasAnyConnection(pCaloHit)) continue;
	  ...
	  {
		  const sdhcal_content::CaloHit *const seed(dynamic_cast<const sdhcal_content::CaloHit *const>(caloHit));
      ...
      float &closestDistance, bool onlyUseConnectedHit)
  {
        ...
        endIter != iter ; ++iter)
    {
	  ...
	  if(onlyUseConnectedHit && !SDHCALContentApi::HasAnyConnection(pCaloHit)) continue;
  ...
  pandora::StatusCode ClusterHelper::GetMeanSurroundingEnergy(const pandora::Cluster *const pCluster, float &meanSurroundingEnergy)
  {
    ...
    for (pandora::OrderedCaloHitList::const_reverse_iterator iter = orderedCaloHitList.rbegin(), iterEnd = orderedCaloHitList.rend(); iter != iterEnd; ++iter)
    {
      for (pandora::CaloHitList::const_iterator hIter = iter->second->begin(), hIterEnd = iter->second->end(); hIter != hIterEnd; ++hIter)
      {
        const sdhcal_content::CaloHit *const pCaloHit(dynamic_cast<const sdhcal_content::CaloHit *const>(*hIter));
  ...
  pandora::StatusCode ClusterHelper::CleanAndDeleteCluster(const pandora::Algorithm &algorithm, const pandora::Cluster *const pCluster)
  {
        ...
        endIter != iter ; ++iter)
    {
      ...
      PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, SDHCALContentApi::RemoveAndDeleteAllConnections(pCaloHit));
    }
