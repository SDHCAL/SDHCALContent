  ...
  pandora::StatusCode ConnectorAlignmentTool::ConnectBackward(const pandora::OrderedCaloHitList &orderedCaloHitList, const Connector *const pConnector) const
  {
    ...
    for(unsigned int pl = startFromPseudoLayer ; pl != endFromPseudoLayer ; --pl)
    {
          ...
          endIter != iter ; ++iter)
      {
        ...
        if(SDHCALContentApi::IsConnected(pConnector->GetFrom(), pCaloHit, BACKWARD_DIRECTION))
          continue;

        PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, SDHCALContentApi::Connect(pConnector->GetFrom(), pCaloHit, BACKWARD_DIRECTION));
      }
    }
  ...
  pandora::StatusCode ConnectorAlignmentTool::ConnectForward(const pandora::OrderedCaloHitList &orderedCaloHitList, const Connector *const pConnector) const
  {
    ...
    for(unsigned int pl = startToPseudoLayer ; pl <= endToPseudoLayer ; ++pl)
    {
      ...
      for(pandora::CaloHitList::const_iterator iter = findIter->second->begin(), endIter = findIter->second->end() ;
          endIter != iter ; ++iter)
      {
        const CaloHit *const pCaloHit = dynamic_cast<const sdhcal_content::CaloHit *const>(*iter);
        ...
        // check if already connected
        if(SDHCALContentApi::IsConnected(pConnector->GetTo(), pCaloHit, FORWARD_DIRECTION))
          continue;

        PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, SDHCALContentApi::Connect(pConnector->GetTo(), pCaloHit, FORWARD_DIRECTION));
      }
    }
