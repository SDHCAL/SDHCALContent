...
namespace sdhcal_content
{
  ...
  pandora::StatusCode ConnectorCleaningTool::Process(const pandora::Algorithm& /*algorithm*/, const pandora::CaloHitList *const /* caloHitList */)
  {
    ...
    const pandora::CaloHitList* pCaloHitList = SDHCALClusteringAlgorithm::GetCaloHitList();
  ...
  pandora::CartesianVector ConnectorCleaningTool::GetReferenceVector(const sdhcal_content::CaloHit *const pCaloHit) const
  {
  ...
  pandora::StatusCode ConnectorCleaningTool::CleanCaloHits(const pandora::CaloHitList *const pCaloHitList) const
  {
    ...
    {
      const sdhcal_content::CaloHit *pCaloHit = dynamic_cast<const sdhcal_content::CaloHit *>(caloHit);

      if(NULL == pCaloHit) return pandora::STATUS_CODE_FAILURE;

      ConnectorList backwardConnectorList(SDHCALContentApi::GetConnectorList(pCaloHit, BACKWARD_DIRECTION));
	  ...
	  for(int iCon = 0; iCon < backwardConnectorVector.size(); ++iCon)
      {
	    ...
	    const unsigned int nConnectors = SDHCALContentApi::GetConnectorList(pFromCaloHit, FORWARD_DIRECTION).size() + 
			100 * SDHCALContentApi::GetConnectorList(pFromCaloHit, BACKWARD_DIRECTION).size();
        ...
        endIter != iter ; ++iter)
    {
      ...
      {
        const sdhcal_content::CaloHit *const pCaloHit = dynamic_cast<const sdhcal_content::CaloHit *const>(*hitIter);
        PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, SDHCALContentApi::RemoveConnectionBetween(iter->first, pCaloHit));
      }
    }
