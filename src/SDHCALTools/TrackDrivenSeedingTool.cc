...
namespace sdhcal_content
{
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------
	const pandora::CartesianVector TrackDrivenSeedingTool::GetFromHitDirection(const sdhcal_content::CaloHit *const pCaloHit)
	{
        pandora::CartesianVector meanBackwardDirection(0, 0, 0);
		const int backwardReferenceDirectionDepth = 2;

		const ConnectorList &backwardConnectorList(SDHCALContentApi::GetConnectorList(pCaloHit, BACKWARD_DIRECTION));
      ...
      pandora::CaloHitVector &caloHitVector)
  {
    ...
    {
      const sdhcal_content::CaloHit *const pCaloHit = dynamic_cast<const sdhcal_content::CaloHit *const>(caloHitVector.at(i));
      ...
      while(1)
      {
        ...
        if(!plIter->second->empty())
        {
          ...
          for(pandora::CaloHitList::const_iterator iter = hitsInRange.begin(), endIter = hitsInRange.end() ;
              endIter != iter ; ++iter)
          {
            const sdhcal_content::CaloHit *const pTestCaloHit = dynamic_cast<const sdhcal_content::CaloHit *const>(*iter);

            if( ! PandoraContentApi::IsAvailable<pandora::CaloHit>( algorithm, pTestCaloHit ) )
              continue;

            // check for existing connection
            if(SDHCALContentApi::IsConnected(pCaloHit, pTestCaloHit, sdhcal_content::FORWARD_DIRECTION))
              continue;
            ...
            PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, SDHCALContentApi::Connect(pCaloHit, pTestCaloHit, sdhcal_content::FORWARD_DIRECTION, 1., creationStage));
