...
namespace sdhcal_content
{
  ...
  pandora::StatusCode ConnectorPlusTool::Process(const pandora::Algorithm &algorithm, const pandora::CaloHitList *const /* pCaloHitList */)
  {
	...
	for(int iHit = 0; iHit < seedHits.size(); ++iHit)
	{
        ...
        const sdhcal_content::CaloHit *const pCaloHitI = dynamic_cast<const sdhcal_content::CaloHit *const>( seedHits.at(iHit) );
        ...
        for(unsigned int pl = maxLayer+1 ; pl <= maxLayer + m_maxPseudoLayerConnection ; ++pl)
        {
          ...
          {
            const sdhcal_content::CaloHit *const pCaloHitJ = dynamic_cast<const sdhcal_content::CaloHit *const>(*iterJ);
            ...
            if(m_shouldDiscriminateConnectedHits && !SDHCALContentApi::GetConnectorList(pCaloHitJ, BACKWARD_DIRECTION).empty())
              continue;
            ...
            // connect !
            PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, SDHCALContentApi::Connect(pCaloHitI, pCaloHitJ, FORWARD_DIRECTION));
	...
	for(int iHit = 0; iHit < seedHitsInHCAL.size(); ++iHit)
	{
        ...
        const sdhcal_content::CaloHit *const pCaloHitI = dynamic_cast<const sdhcal_content::CaloHit *const>( seedHitsInHCAL.at(iHit) );
